---
title: "From API to app: a small ML pipeline for Kiva loan descriptions"
jupyter: kiva-ml
---
...
::: {.callout-important}
This pipeline predicts a coarse sector label, nothing more. It says nothing about creditworthiness, repayment risk, or whether a loan should be approved, and it must never be used as an input to those decisions. The borrowers behind this data are largely women running microenterprises, alongside refugee and elderly clients, and any extension of this work beyond illustration would need a much closer look at fairness, consent, and the downstream effects of a wrong label. I use this example typically for illustration purposes.
:::
...
The extraction step (`kiva_extract.py` in the repo) loops over Kiva's `loans/search` endpoint and collects four pages of 500 loans each, filtered to Uganda. Requests without a browser style `User-Agent` header are rejected with a 403, so that header is set explicitly.
